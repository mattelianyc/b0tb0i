...
const puppet = require( './main/puppet' );
``
// open a window
const openWindow = () => {
...
// HANDLE M.E.
ipcMain.handle( 'app:run-bot', ( event ) => {
  console.log('run bot')
  puppet.runBot()
})

ipcMain.handle( 'app:open-bot-config', ( event ) => {
  console.log('open bot config window')
  openWindow();

  puppet.openBotConfig()
})
...
// LISTEN TO M.E.
// ipcMain.on('app:run-bot', ( event ) => {
//     console.log('run bot event  ', event)
//     puppet.runBot()
// })
